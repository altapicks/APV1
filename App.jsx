...
function BuilderTab({ players: rp }) {
  ...
  const sp = useMemo(() => [...rp].sort((a, b) => b.val - a.val), [rp]);
...
function ExportTab({ players }) {
  ...
  const eDK = () => { const pd = sp.map(p => ({ name: p.name, salary: p.salary, id: p.id, projection: p.proj, opponent: p.opponent, maxExp: 60, minExp: 0 })); const r = optimize(pd, 45, 50000, 6); let c = 'P,P,P,P,P,P\n'; r.lineups.forEach(lu => { const ps = lu.players.map(i => pd[i]).sort((a, b) => b.salary - a.salary); c += ps.map(p => p.id).join(',') + '\n'; }); dl(c, 'dk_upload.csv'); };
  const eR = () => { const pd = sp.map(p => ({ name: p.name, salary: p.salary, id: p.id, projection: p.proj, opponent: p.opponent, maxExp: 60, minExp: 0 })); const r = optimize(pd, 45, 50000, 6); let c = 'Rank,Proj,Salary,P1,P2,P3,P4,P5,P6\n'; r.lineups.forEach((lu, i) => { const ps = lu.players.map(j => pd[j]).sort((a, b) => b.salary - a.salary); c += `${i + 1},${lu.proj},${lu.sal},${ps.map(p => p.name).join(',')}\n`; }); dl(c, 'lineups.csv'); };
